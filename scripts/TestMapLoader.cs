...
{
	private PackedScene partPrefab;

	private RobloxFile data;

	private Dictionary<int, int> SurfaceMappings = new()
	{
	...
	{
		partPrefab = GD.Load<PackedScene>("res://scenes/prefabs/part.tscn");
		...
		data = RobloxFile.Open(file.GetBuffer((long)file.GetLength()));
	...
	{
		foreach (var descendent in data.GetDescendantsOfType<RobloxFiles.Part>())
		{
			var newPart = partPrefab.Instantiate<Part>();
			...
			for (var i = 0; i < 6; i++)
				newPart.Surfaces[i] = SurfaceMappings.GetValueOrDefault(robloxSurfaces[i], 0);
